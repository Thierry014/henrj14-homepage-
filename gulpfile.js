...
gulp.task('js_production_minified', function () {
	return gulp.src(page.js)
		.pipe(webpack({
			...
			plugins: [
				new terser({cache: true, parallel: true, terserOptions: {output: {comments: false}}})
			]
		}))
		.pipe(gulp.dest(dir.js));
});
...
gulp.task('distCopy', function () {
	return gulp.src(['src/**/*', '!src/assets/{js/src,plugin/thesaas,scss}{,/**}']).pipe(gulp.dest('web/assets/dist/'));
});
